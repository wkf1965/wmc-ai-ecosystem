import { useEffect, useMemo, useState } from 'react'
import {
  AlertTriangle,
  BellRing,
  ClipboardPlus,
  Download,
  Moon,
  ShieldAlert,
  Sparkles,
  CheckCircle2,
  ClipboardCheck,
  TrendingDown,
} from 'lucide-react'
import { Link } from 'react-router-dom'
import { PageHeader, Card, Badge } from '../components/ui'
import {
  appendSleepObservationNote,
  bumpSleepMonitoringScore,
  mergeSleepMonitoringInstances,
  upsertSleepMonitoringInstance,
} from '../db/sleepMonitoringLoopStorage.js'
import {
  buildSleepLoopAiAlerts,
  buildSleepReportCsv,
  deriveSleepRiskBand,
  formatObsTime,
  listSleepMonitoringRows,
  sleepLoopAiSummary,
  sleepMonitoringBucket,
  sleepScoreTotalsDisplay,
} from '../lib/sleepMonitoringLoopSimulation.js'
import { usePatients } from '../hooks/usePatients.js'

const btn =
  'min-h-[44px] touch-manipulation rounded-xl px-3 py-2.5 text-xs font-semibold shadow-sm transition-colors active:scale-[0.98] disabled:pointer-events-none disabled:opacity-45'
const btnPrimary = `${btn} bg-indigo-600 text-white hover:bg-indigo-700`
const btnMuted = `${btn} border border-slate-200 bg-white text-slate-700 hover:bg-slate-50`
const btnWarn = `${btn} border border-amber-200 bg-amber-50 text-amber-900 hover:bg-amber-100`
const btnDanger = `${btn} border border-rose-200 bg-rose-50 text-rose-900 hover:bg-rose-100`
const btnSuccess = `${btn} border border-emerald-200 bg-emerald-50 text-emerald-900 hover:bg-emerald-100`

const COLS = [
  { key: 'poor_sleep', title: 'Poor sleep', sub: 'Fragmented rest', badge: 'warning' },
  { key: 'night_wandering', title: 'Night wandering', sub: 'Egress risk focus', badge: 'danger' },
  { key: 'agitation', title: 'Agitation', sub: 'Behaviour overnight', badge: 'danger' },
  { key: 'confusion_risk', title: 'Confusion risk', sub: 'Delirium cues', badge: 'warning' },
  { key: 'stable_sleep', title: 'Stable sleep', sub: 'Within tolerance', badge: 'success' },
]

function stripDerived(row) {
  const { bucket: _b, riskBand: _r, ...rest } = row
  return rest
}

function downloadText(filename, text) {
  const blob = new Blob([text], { type: 'text/csv;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = filename
  a.click()
  URL.revokeObjectURL(url)
}

function SleepCard({ row, selected, onSelect }) {
  const band = row.riskBand || deriveSleepRiskBand(row)
  const bucket = row.bucket || sleepMonitoringBucket(row)
  const bucketLabel =
    bucket === 'poor_sleep'
      ? 'Poor sleep'
      : bucket === 'night_wandering'
        ? 'Night wandering'
        : bucket === 'agitation'
          ? 'Agitation'
          : bucket === 'confusion_risk'
            ? 'Confusion risk'
            : 'Stable sleep'

  return (
    <Card
      role="button"
      tabIndex={0}
      onClick={() => onSelect(row.patientId)}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault()
          onSelect(row.patientId)
        }
      }}
      className={`cursor-pointer border shadow-sm transition-shadow ${selected ? 'ring-2 ring-indigo-500 ring-offset-2' : 'border-slate-100'}`}
      padding="p-4"
    >
      <div className="flex flex-wrap items-start justify-between gap-2 border-b border-slate-100 pb-2">
        <div className="min-w-0">
          <p className="truncate text-base font-bold text-slate-900">{row.patientName}</p>
          <p className="text-xs text-slate-600">
            Rm <span className="font-semibold">{row.roomNumber}</span>
          </p>
        </div>
        <div className="flex flex-wrap justify-end gap-1">
          <Badge variant={band.variant}>{band.label}</Badge>
          <Badge variant={bucket === 'stable_sleep' ? 'success' : bucket === 'poor_sleep' ? 'warning' : 'info'}>{bucketLabel}</Badge>
        </div>
      </div>

      <dl className="mt-3 space-y-1 text-xs text-slate-700">
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Sleep window</dt>
          <dd className="font-semibold">
            {row.sleepStartTime} → {row.wakeTime}
          </dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Total sleep</dt>
          <dd className="tabular-nums font-bold text-slate-900">{Number(row.totalSleepHours).toFixed(1)} h</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Night wakings</dt>
          <dd className="tabular-nums font-semibold">{row.nightWakingEpisodes}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Flags</dt>
          <dd className="flex max-w-[65%] flex-wrap justify-end gap-1">
            {row.wanderingBehavior ? (
              <Badge variant="danger">Wandering</Badge>
            ) : null}
            {row.agitationAtNight ? (
              <Badge variant="warning">Agitation</Badge>
            ) : null}
            {row.confusionAtNight ? (
              <Badge variant="warning">Confusion</Badge>
            ) : null}
            {row.painComplaint ? (
              <Badge variant="info">Pain</Badge>
            ) : null}
            {!row.wanderingBehavior &&
            !row.agitationAtNight &&
            !row.confusionAtNight &&
            !row.painComplaint ? (
              <span className="text-slate-400">—</span>
            ) : null}
          </dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Toilet visits</dt>
          <dd className="tabular-nums">{row.toiletVisits}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Nurse assigned</dt>
          <dd className="max-w-[58%] text-right font-medium">{row.nurseAssigned}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Last observation</dt>
          <dd className="text-right">{formatObsTime(row.lastNightObservationTime)}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">Next due</dt>
          <dd className="text-right font-medium text-indigo-800">{formatObsTime(row.nextObservationDueTime)}</dd>
        </div>
        {row.behaviorEscalated ? (
          <p className="rounded-lg bg-rose-50 px-2 py-1 text-[11px] font-semibold text-rose-900">Behaviour escalated (sim)</p>
        ) : null}
        {Array.isArray(row.observationNotes) && row.observationNotes.length > 0 ? (
          <div className="rounded-lg bg-slate-50 px-2 py-1.5 text-[11px] text-slate-700">
            <p className="font-semibold text-slate-600">Latest note</p>
            <p className="mt-0.5 whitespace-pre-wrap">{row.observationNotes[row.observationNotes.length - 1]?.text}</p>
          </div>
        ) : null}
      </dl>
    </Card>
  )
}

export default function SleepMonitoringLoopPage() {
  const { patients } = usePatients()
  const [tick, setTick] = useState(0)
  const [toast, setToast] = useState(null)
  const [selectedId, setSelectedId] = useState(null)
  const [mobileCol, setMobileCol] = useState('poor_sleep')

  useEffect(() => {
    mergeSleepMonitoringInstances(patients)
  }, [patients])

  useEffect(() => {
    function bump() {
      setTick((t) => t + 1)
    }
    window.addEventListener('wmc-sleep-monitoring-loop-updated', bump)
    return () => window.removeEventListener('wmc-sleep-monitoring-loop-updated', bump)
  }, [])

  const rows = useMemo(() => listSleepMonitoringRows(patients), [patients, tick])

  const alerts = useMemo(() => buildSleepLoopAiAlerts(rows.map(stripDerived)), [rows])
  const summary = useMemo(() => sleepLoopAiSummary(rows.map(stripDerived)), [rows])
  const scores = useMemo(() => sleepScoreTotalsDisplay(), [tick])

  const buckets = useMemo(() => {
    return {
      poor_sleep: rows.filter((r) => r.bucket === 'poor_sleep'),
      night_wandering: rows.filter((r) => r.bucket === 'night_wandering'),
      agitation: rows.filter((r) => r.bucket === 'agitation'),
      confusion_risk: rows.filter((r) => r.bucket === 'confusion_risk'),
      stable_sleep: rows.filter((r) => r.bucket === 'stable_sleep'),
    }
  }, [rows])

  const selected = rows.find((r) => r.patientId === selectedId) || null

  function showToast(msg, tone = 'info') {
    setToast({ msg, tone })
    window.setTimeout(() => setToast(null), 2600)
  }

  function requireSelection() {
    if (!selected) {
      showToast('Select a resident card first.', 'warn')
      return null
    }
    return selected
  }

  function handleSleepCheck() {
    const row = requireSelection()
    if (!row) return
    const hoursRaw = window.prompt('Total sleep hours (last night)', String(row.totalSleepHours))
    if (hoursRaw === null) return
    const wakingsRaw = window.prompt('Night waking episodes', String(row.nightWakingEpisodes))
    if (wakingsRaw === null) return
    const hours = parseFloat(hoursRaw)
    const wakings = parseInt(String(wakingsRaw).replace(/\D/g, ''), 10)
    if (!Number.isFinite(hours) || hours < 0 || hours > 16) {
      showToast('Enter sleep hours between 0–16.', 'warn')
      return
    }
    if (!Number.isFinite(wakings) || wakings < 0 || wakings > 24) {
      showToast('Enter waking episodes 0–24.', 'warn')
      return
    }
    const now = Date.now()
    upsertSleepMonitoringInstance(row.patientId, {
      ...stripDerived(row),
      totalSleepHours: hours,
      nightWakingEpisodes: wakings,
      lastNightObservationTime: new Date(now).toISOString(),
      nextObservationDueTime: new Date(now + 90 * 60 * 1000).toISOString(),
    })
    bumpSleepMonitoringScore('monitor', 1)
    showToast('Sleep check recorded (simulation).', 'success')
  }

  function handleAddObservation() {
    const row = requireSelection()
    if (!row) return
    const text = window.prompt(`Night observation — ${row.patientName}`, 'Restless period · repositioned · offered fluids')
    if (text === null) return
    appendSleepObservationNote(row.patientId, text)
    const now = Date.now()
    upsertSleepMonitoringInstance(row.patientId, {
      lastNightObservationTime: new Date(now).toISOString(),
      nextObservationDueTime: new Date(now + 60 * 60 * 1000).toISOString(),
    })
    if (text.trim()) showToast('Observation saved.')
  }

  function handleEscalate() {
    const row = requireSelection()
    if (!row) return
    upsertSleepMonitoringInstance(row.patientId, {
      ...stripDerived(row),
      behaviorEscalated: true,
      agitationAtNight: true,
    })
    bumpSleepMonitoringScore('highRisk', 1)
    showToast('Behaviour concern escalated (simulation).', 'warn')
  }

  function handleGenerateReport() {
    const csv = buildSleepReportCsv(rows.map(stripDerived))
    downloadText(`sleep-monitoring-${new Date().toISOString().slice(0, 10)}.csv`, csv)
    showToast('Sleep report CSV downloaded.')
  }

  function handleMarkStable() {
    const row = requireSelection()
    if (!row) return
    upsertSleepMonitoringInstance(row.patientId, {
      ...stripDerived(row),
      wanderingBehavior: false,
      agitationAtNight: false,
      confusionAtNight: false,
      painComplaint: false,
      behaviorEscalated: false,
      nightWakingEpisodes: Math.min(1, Number(row.nightWakingEpisodes) || 0),
      totalSleepHours: Math.max(7, Number(row.totalSleepHours) || 7),
    })
    bumpSleepMonitoringScore('good', 1)
    showToast('Marked stable pathway (simulation).', 'success')
  }

  return (
    <div className="mx-auto max-w-[1600px] pb-8">
      <PageHeader
        title="Sleep monitoring loop"
        description="Nocturnal surveillance board with fragmented sleep, wandering, and delirium cues — simulation only; align with facility sleep & behaviour policies."
        action={
          <div className="flex flex-wrap gap-2">
            <Badge variant="info">Simulation mode</Badge>
            <Link
              to="/mental-health-loop"
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-50"
            >
              Mental health loop
            </Link>
          </div>
        }
      />

      {toast ? (
        <div
          role="status"
          className={`mb-4 rounded-xl border px-4 py-3 text-sm font-semibold ${
            toast.tone === 'success'
              ? 'border-emerald-200 bg-emerald-50 text-emerald-900'
              : toast.tone === 'warn'
                ? 'border-amber-200 bg-amber-50 text-amber-900'
                : 'border-sky-200 bg-sky-50 text-sky-900'
          }`}
        >
          {toast.msg}
        </div>
      ) : null}

      <Card className="mb-3" padding="p-4">
        <div className="flex flex-wrap gap-2">
          <button type="button" className={btnPrimary} onClick={handleSleepCheck} disabled={!selected}>
            <span className="inline-flex items-center gap-1">
              <ClipboardCheck className="h-4 w-4 shrink-0" aria-hidden />
              Record sleep check
            </span>
          </button>
          <button type="button" className={btnMuted} onClick={handleAddObservation} disabled={!selected}>
            <span className="inline-flex items-center gap-1">
              <ClipboardPlus className="h-4 w-4 shrink-0" aria-hidden />
              Add night observation
            </span>
          </button>
          <button type="button" className={btnWarn} onClick={handleEscalate} disabled={!selected}>
            <span className="inline-flex items-center gap-1">
              <ShieldAlert className="h-4 w-4 shrink-0" aria-hidden />
              Escalate behavior concern
            </span>
          </button>
          <button type="button" className={btnMuted} onClick={handleGenerateReport}>
            <span className="inline-flex items-center gap-1">
              <Download className="h-4 w-4 shrink-0" aria-hidden />
              Generate sleep report
            </span>
          </button>
          <button type="button" className={btnSuccess} onClick={handleMarkStable} disabled={!selected}>
            <span className="inline-flex items-center gap-1">
              <CheckCircle2 className="h-4 w-4 shrink-0" aria-hidden />
              Mark stable
            </span>
          </button>
        </div>
        <p className="mt-3 text-xs text-slate-500">
          Tap a resident card to apply bedside actions. Observation cadence updates automatically in demo mode.
        </p>
      </Card>

      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Card className="relative overflow-hidden" padding="p-4">
          <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-linear-to-br from-indigo-400 to-violet-600 opacity-15 blur-2xl" />
          <div className="relative flex items-start gap-2">
            <TrendingDown className="h-5 w-5 shrink-0 text-indigo-600" aria-hidden />
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Poor sleep roster</p>
              <p className="mt-1 text-sm font-semibold leading-snug text-slate-900">{summary.patientsWithPoorSleep}</p>
            </div>
          </div>
        </Card>
        <Card className="relative overflow-hidden" padding="p-4">
          <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-linear-to-br from-orange-400 to-rose-600 opacity-15 blur-2xl" />
          <div className="relative flex items-start gap-2">
            <Moon className="h-5 w-5 shrink-0 text-orange-600" aria-hidden />
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Night fall risk</p>
              <p className="mt-1 text-sm font-semibold leading-snug text-slate-900">{summary.nightFallRisk}</p>
            </div>
          </div>
        </Card>
        <Card className="relative overflow-hidden sm:col-span-2" padding="p-4">
          <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-linear-to-br from-teal-400 to-cyan-600 opacity-15 blur-2xl" />
          <div className="relative flex items-start gap-2">
            <Sparkles className="h-5 w-5 shrink-0 text-teal-600" aria-hidden />
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Wandering behaviour</p>
              <p className="mt-1 text-sm leading-relaxed text-slate-800">{summary.wanderingBehavior}</p>
            </div>
          </div>
        </Card>
      </div>

      <Card className="mt-3" padding="p-4 sm:p-5">
        <div className="flex items-center gap-2">
          <Moon className="h-5 w-5 text-indigo-600" aria-hidden />
          <h3 className="text-sm font-semibold text-slate-900">AI summary</h3>
        </div>
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-100 bg-slate-50/80 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Nurse action checklist</p>
            <p className="mt-1 text-sm leading-relaxed text-slate-800">{summary.nurseActionChecklist}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-slate-50/80 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Doctor / counsellor review</p>
            <p className="mt-1 text-sm leading-relaxed text-slate-800">{summary.doctorCounsellorReview}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-slate-50/80 p-3 lg:col-span-2">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Family update suggestion</p>
            <p className="mt-1 text-sm leading-relaxed text-slate-800">{summary.familyUpdateSuggestion}</p>
          </div>
        </div>
      </Card>

      <Card className="mt-4" padding="p-4 sm:p-5">
        <div className="flex items-center gap-2">
          <Moon className="h-5 w-5 text-teal-600" aria-hidden />
          <h3 className="text-sm font-semibold text-slate-900">Sleep scoring</h3>
        </div>
        <p className="mt-0.5 text-xs text-slate-500">Simulation tally · includes demo baseline · updates with nurse actions</p>
        <dl className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-5">
          {[
            { label: 'Good', val: scores.good },
            { label: 'Monitor', val: scores.monitor },
            { label: 'Disturbed', val: scores.disturbed },
            { label: 'High risk', val: scores.highRisk },
            { label: 'Urgent review', val: scores.urgentReview },
          ].map((x) => (
            <div key={x.label} className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
              <dt className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{x.label}</dt>
              <dd className="text-xl font-bold tabular-nums text-slate-900">{x.val}</dd>
            </div>
          ))}
        </dl>
      </Card>

      <Card className="mt-4" padding="p-4 sm:p-6">
        <div className="mb-3 flex items-center gap-2">
          <BellRing className="h-5 w-5 text-amber-600" aria-hidden />
          <div>
            <h3 className="text-base font-semibold text-slate-900">AI alerts</h3>
            <p className="text-sm text-slate-500">Disturbance · wandering · delirium · falls · pain · MD review</p>
          </div>
        </div>
        {alerts.length === 0 ? (
          <p className="rounded-xl border border-emerald-100 bg-emerald-50/80 px-4 py-4 text-sm text-emerald-900">
            No overnight escalation alerts on this roster snapshot.
          </p>
        ) : (
          <ul className="grid gap-2 lg:grid-cols-2">
            {alerts.map((a) => (
              <li key={a.id} className="flex gap-2 rounded-xl border border-slate-100 bg-slate-50/90 px-3 py-2.5 text-sm">
                <AlertTriangle
                  className={`mt-0.5 h-4 w-4 shrink-0 ${
                    a.severity === 'critical' ? 'text-red-600' : a.severity === 'high' ? 'text-orange-600' : 'text-amber-600'
                  }`}
                  aria-hidden
                />
                <div className="min-w-0">
                  <p className="font-semibold text-slate-900">{a.title}</p>
                  <p className="text-xs font-medium uppercase tracking-wide text-slate-500">{a.category}</p>
                  <p className="mt-0.5 text-xs text-slate-600">{a.detail}</p>
                </div>
              </li>
            ))}
          </ul>
        )}
      </Card>

      <div className="mt-6 xl:hidden">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Sleep monitoring board</p>
        <div className="flex gap-1 overflow-x-auto pb-2 [-webkit-overflow-scrolling:touch]">
          {COLS.map((c) => (
            <button
              key={c.key}
              type="button"
              className={`shrink-0 rounded-full px-3 py-2 text-xs font-semibold ring-1 ring-inset transition-colors ${
                mobileCol === c.key ? 'bg-indigo-600 text-white ring-indigo-700' : 'bg-white text-slate-600 ring-slate-200'
              }`}
              onClick={() => setMobileCol(c.key)}
            >
              <span className="line-clamp-2 max-w-[132px] text-left">{c.title}</span>
              <span className="ml-1 tabular-nums opacity-80">({buckets[c.key].length})</span>
            </button>
          ))}
        </div>
        <div className="space-y-3">
          {buckets[mobileCol].map((row) => (
            <SleepCard key={row.patientId} row={row} selected={selectedId === row.patientId} onSelect={setSelectedId} />
          ))}
          {buckets[mobileCol].length === 0 ? (
            <p className="rounded-xl border border-slate-100 bg-slate-50 py-8 text-center text-sm text-slate-600">
              No residents in this column.
            </p>
          ) : null}
        </div>
      </div>

      <div className="mt-6 hidden gap-3 xl:grid xl:grid-cols-5">
        {COLS.map((col) => (
          <div key={col.key} className="flex min-h-0 flex-col rounded-2xl border border-slate-200/80 bg-slate-50/50">
            <div className="shrink-0 border-b border-slate-200/80 px-3 py-3">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-xs font-bold leading-tight text-slate-900">{col.title}</h3>
                <Badge variant={col.badge}>{buckets[col.key].length}</Badge>
              </div>
              <p className="mt-0.5 text-[11px] text-slate-500">{col.sub}</p>
            </div>
            <div className="max-h-[calc(100vh-13rem)] min-h-[220px] space-y-3 overflow-y-auto overscroll-contain p-3">
              {buckets[col.key].map((row) => (
                <SleepCard key={row.patientId} row={row} selected={selectedId === row.patientId} onSelect={setSelectedId} />
              ))}
              {buckets[col.key].length === 0 ? (
                <p className="py-8 text-center text-xs text-slate-500">Empty</p>
              ) : null}
            </div>
          </div>
        ))}
      </div>
    </div>
  )
}
